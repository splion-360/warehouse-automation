...
class VocDataset(Dataset):
    def __init__(self, anno_file_type, img_size=416, vis=False):
    ...
    def __getitem__(self, item):
        ...
        if (self.vis):
            img, bboxes = dataAug.Mixup(flag=False)(img_org, bboxes_org, img_mix, bboxes_mix)
        else:
...
if __name__ == "__main__":
    ...
    for i, (img, label_sbbox, label_mbbox, label_lbbox, sbboxes, mbboxes, lbboxes) in enumerate(dataloader):
        if i<=10:
            ...
            if img.shape[0] == 1:
                labels = np.concatenate([label_sbbox.reshape(-1, 7), label_mbbox.reshape(-1, 7),
                                         label_lbbox.reshape(-1, 7)], axis=0)
