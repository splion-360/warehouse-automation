...
from model.head.yolo_head import gtid
import wandb
...
class Trainer(object):
    def __init__(self,  weight_path, resume, gpu_id, is_wandb = False, vis = False):
        ...
        self.train_dataloader = DataLoader(self.train_dataset,
                                           ...
                                           shuffle=True)
        if (not vis):
            self.yolov3 = Yolov3(cfg).to(self.device)
        ...
        if is_wandb:
            ...
            self.run = wandb.init(project="peer-robotics", ## Name of the project 
                            config = {  "dataset":"Pallet (Floor Segmentation)",
                                        ...
                                        name="Training Object Detection Model" # Name of the session
                    )
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, (imgs, label_sbbox, label_mbbox, label_lbbox, sbboxes, mbboxes, lbboxes)  in enumerate(self.train_dataloader):
                ...
                if i%20==0:
                    if self.is_wandb: 
                        wandb.log(data = {'train loss' : loss.item(), 'GIoU loss': loss_giou.item(), 'class loss': loss_cls.item(), 'obj loss': loss_conf.item()})
            ...
            self.yolov3.eval()
            with torch.no_grad():
                APs = Evaluator(self.yolov3, False).APs_voc()
                for i in APs:
                    print("{} --> mAP : {}".format(i, APs[i]))
                    mAP += APs[i]
                mAP = mAP / self.train_dataset.num_classes
                if self.is_wandb:
                    wandb.log(data = {'mAP': mAP})
